...
export default class Template {
  ...
  _uiHandleDarkMode(mode = 'init') {
    ...
    if (mode === 'init' && !self._lPage.classList.contains('dark-mode')) {
      if (self._lPage.classList.contains('sidebar-dark')) {
        localStorage.setItem('untitleuiDefaultsSidebarDark', true);
      } else {
        localStorage.removeItem('untitleuiDefaultsSidebarDark');
      }

      if (self._lPage.classList.contains('page-header-dark')) {
        localStorage.setItem('untitleuiDefaultsPageHeaderDark', true);
      } else {
        localStorage.removeItem('untitleuiDefaultsPageHeaderDark');
      }
    }

    // If remember-theme class is added in #page-container
    if (this._lPage.classList.contains('remember-theme')) {
      let darkMode = localStorage.getItem('untitleuiDarkMode') || false;
      ...
      if (mode === 'init') {
        ...
        localStorage.setItem('untitleuiDarkMode', true);
      } else if (mode === 'off') {
        localStorage.removeItem('untitleuiDarkMode');
      }
    } else if (mode === 'init') {
      localStorage.removeItem('untitleuiDarkMode');
    }
  ...
  _uiHandleTheme() {
    ...
    if (rememberTheme) {
      let themeName = localStorage.getItem('untitleuiThemeName') || false;
      ...
      localStorage.removeItem('untitleuiThemeName');
    }
    ...
    document.querySelectorAll('[data-toggle="theme"]').forEach(el => {
      el.addEventListener('click', e => {
        ...
        if (rememberTheme) {
          localStorage.setItem('untitleuiThemeName', themeName);
        }
      });
    });
  }
  ...
  _uiApiLayout(mode = 'init') {
    ...
    let layoutAPI = {
      ...
      dark_mode_off: () => {
        if (!localStorage.getItem('untitleuiDefaultsSidebarDark')) {
          self._lPage.classList.remove('sidebar-dark');
        }

        if (!localStorage.getItem('untitleuiDefaultsPageHeaderDark')) {
          self._lPage.classList.remove('page-header-dark');
        }
